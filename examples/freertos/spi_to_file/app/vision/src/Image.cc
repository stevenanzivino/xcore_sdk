...
namespace vision
{
  ...
  Image::Image(int rows_, int cols_, int chans_) : mchans(chans_),
                                                   mcols(cols_),
                                                   mrows(rows_),
                                                   mcol_stride(chans_),
  ...
  sample_t *Image::get_pixel(int row, int col)
  {
    int tmp = get_sample_ind(row, col);
    return image_data->data() + tmp;
  }

  int Image::get_sample_ind(int row, int col)
  {
    return row * mrow_stride + col * mcol_stride;
  }
  int Image::get_sample_ind(RowCol rc)
  {
    return rc.row * mrow_stride + rc.col * mcol_stride;
  }

  bool Image::is_valid(Region r)
  {
